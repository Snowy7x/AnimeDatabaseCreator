...
export const scrapeSource = async (serverId) => {
  ...
  const rapidAjax = await axios.get(
    `https://rapid-cloud.co/ajax/embed-6/getSources?id=${rapidId}`,
    {
      headers: headerOption,
    }
  );
  ...
  let decryptKey = await axios.get(
    "https://github.com/enimax-anime/key/blob/e6/key.txt"
  );

  const $ = load(decryptKey.data);

  const source = CryptoJS.AES.decrypt(sources, $("#LC1").text()).toString(
    CryptoJS.enc.Utf8
  );
  return {
